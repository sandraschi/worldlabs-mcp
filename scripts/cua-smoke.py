...
def check_diagnostics():
    try:
        ...
        if data.get("success"):
            ...
            sys_info = d["system"]
            log(
                f"System: CPU {sys_info.get('cpu_percent')}% | "
                f"Mem {sys_info.get('memory_percent')}% | "
                f"Disk {sys_info.get('disk_percent')}%"
            )
...
def nav_click_through(output_dir: str):
    ...
    for label, expected_header in nav_routes:
        try:
            idx = next((i for i, (lab, _) in enumerate(nav_routes) if lab == label), 0)
