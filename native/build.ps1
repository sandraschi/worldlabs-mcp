...
if (Test-Path $apiFile) {
    ...
    if ($apiContent -match "127.0.0.1:(\d+)") {
        ...
        if ($apiPort -ne 10865) {
            throw "API_BASE in $apiFile points to port $apiPort but backend serves on 10865. In dev Vite proxies work, in prod/NSIS this gives 'Failed to fetch'."
        }
...
# Step 2: PyInstaller backend (onefile) -- MUST use venv PyInstaller, not global uv tool
Write-Host "-> [2/4] PyInstaller backend..." -ForegroundColor Yellow
$specFile = "$Root\${RepoName}-backend.spec"
if (Test-Path $specFile) {
    Push-Location $Root

    # Ensure pyinstaller is in project venv (not global uv tool -- that can't see project deps)
    ...
    # Run from project venv, NOT global uv tool (global tool can't see project deps)
    ...
    $testPort = 11999
    $oldPort = $env:WORLDLABS_MCP_PORT; $oldHost = $env:WORLDLABS_MCP_HOST
    $env:WORLDLABS_MCP_PORT = "$testPort"; $env:WORLDLABS_MCP_HOST = "127.0.0.1"
    $testProc = Start-Process -FilePath "$Root\dist\${RepoName}-backend.exe" -NoNewWindow -PassThru -RedirectStandardError "$Root\dist\pyi-crash.log"
    Start-Sleep -Seconds 5
    $env:WORLDLABS_MCP_PORT = $oldPort; $env:WORLDLABS_MCP_HOST = $oldHost
...
$strayExe = "$PSScriptRoot\target\release\worldlabs-mcp-backend.exe"
