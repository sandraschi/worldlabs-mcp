...
const BACKEND_NAME: &str = "worldlabs-mcp-backend.exe";
const BACKEND_PORT: u16 = 10865;
...
fn dev_backend_path() -> Option<PathBuf> {
    ...
    let path = PathBuf::from(env!("CARGO_MANIFEST_DIR"))
        .join("binaries")
        .join("worldlabs-mcp-backend-x86_64-pc-windows-msvc.exe");
...
fn resolve_bundled_backend(app: &AppHandle) -> Result<PathBuf, String> {
    ...
    if let Ok(path) = app
        .path()
        .resolve("resources/worldlabs-mcp-backend.exe", BaseDirectory::Resource)
    {
        tried.push(path.display().to_string());
        if path.exists() {
            return Ok(path);
...
pub fn spawn_backend(app: AppHandle, state: &BackendProcess) -> Result<String, String> {
    ...
    command
        .current_dir(&workdir)
        .env("WORLDLABS_MCP_PORT", BACKEND_PORT.to_string())
        .env("WORLDLABS_MCP_HOST", "127.0.0.1")
        .env("WORLDLABS_TAURI", "1")
        // Use Stdio::null() for stdout/stderr to prevent pipe buffer deadlock
        // (Windows pipe buffer is 4KB; uvicorn access logs fill it after ~40 requests)
        .stdout(Stdio::null())
        .stderr(Stdio::null());
