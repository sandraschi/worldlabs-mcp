            ...
            {expanded && detail !== 'quick' && (
                <div className="mt-2 space-y-2 pl-5 border-l border-white/[0.06]">
                    ...
                    {detail === 'verbose' && tool.docstring && (
                        <p className="text-xs text-slate-400 leading-relaxed">{tool.docstring}</p>
                    )}
                    {detail === 'verbose' && tool.example && (
                        <div>
                            <p className="text-[10px] uppercase tracking-wider text-slate-600 mb-1">Example</p>
                            <pre className="text-[11px] font-mono text-slate-300 bg-black/30 rounded p-2 overflow-x-auto whitespace-pre-wrap">{tool.example}</pre>
                        </div>
                    )}
...
const TOOLS: Tool[] = [
    {
        name: 'generate_world_from_text', group: 'generate',
        description: 'Generate a 3D world from a text description',
        args: { text_prompt: 'str — scene description', display_name: 'str (optional)', model: "'Marble 0.1-mini' (default) or 'Marble 0.1-plus'" },
        docstring: 'Submits a text-to-world request. Returns immediately with an in-progress operation. Poll with get_operation or block with wait_for_world.',
    ...
    {
        name: 'generate_world_from_image', group: 'generate',
        description: 'Generate a 3D world from a public image URL',
        args: { image_url: 'str — public URL (jpg, jpeg, png, webp)', text_prompt: 'str (optional)', is_panorama: 'bool — True for 360-degree equirectangular images', model: 'str' },
    ...
    {
        name: 'generate_world_from_multi_image', group: 'generate',
        description: 'Generate from multiple images at specified azimuth angles',
        args: { image_urls: 'list[str] — public image URLs', azimuths_deg: 'list[float] — angle per image, 0-360' },
        docstring: 'Reconstructs a 3D scene from multiple views. 2-8 images at known azimuth angles. Cardinal directions (0/90/180/270) work well.',
    ...
    {
        name: 'generate_world_from_video', group: 'generate',
        description: 'Generate a 3D world from a public video URL',
        args: { video_url: 'str — public URL (mp4, mov, mkv)', text_prompt: 'str (optional)' },
    ...
    {
        name: 'upload_and_generate', group: 'upload',
        description: 'Upload a local file and generate end-to-end',
        args: { local_file_path: 'str — absolute path on disk', kind: "'image' or 'video'", text_prompt: 'str (optional)', is_panorama: 'bool (image only)' },
        docstring: 'Handles signed GCS upload then generation in one call. Preferred over the two-step manual flow for local files.',
        example: "upload_and_generate(\n  local_file_path=\"D:/photos/garden.jpg\",\n  kind=\"image\"\n)",
        notes: 'Supported: jpg, jpeg, png, webp (image); mp4, mov, mkv (video).',
    },
    {
        name: 'prepare_media_upload', group: 'upload',
        description: 'Get a signed GCS upload URL for manual file upload',
        args: { file_name: 'str', kind: "'image' or 'video'", extension: 'str — without dot' },
        docstring: 'Step 1 of the manual upload flow. PUT the raw bytes to upload_info.upload_url, then call generate_world_from_media_asset.',
        example: "prepare_media_upload(file_name=\"photo.jpg\", kind=\"image\", extension=\"jpg\")",
        notes: 'Signed URLs expire after ~15 minutes.',
    },
    {
        name: 'generate_world_from_media_asset', group: 'generate',
        description: 'Generate from a previously uploaded media asset ID',
        args: { media_asset_id: 'str — ID from prepare_media_upload', kind: "'image' or 'video'" },
    ...
    {
        name: 'get_operation', group: 'poll',
        description: 'Single poll of an operation status',
        args: { operation_id: 'str — from any generate call' },
        docstring: 'One-shot poll. Returns immediately with current state. Call in a loop for Marble 0.1-plus jobs (~5 min).',
        example: 'get_operation("op-abc123")',
        notes: 'Preferred over wait_for_world for Marble 0.1-plus to avoid MCP timeouts.',
    },
    {
        name: 'wait_for_world', group: 'poll',
        description: 'Blocking poll until operation completes or times out',
        args: { operation_id: 'str', poll_interval_seconds: 'int (default 15)', timeout_seconds: 'int (default 90)' },
        docstring: 'Blocks until done or timeout. Raises RuntimeError on API failure, TimeoutError if timeout elapses.',
        example: 'wait_for_world("op-abc123", poll_interval_seconds=10, timeout_seconds=90)',
        notes: 'Default 90s is safe for Marble 0.1-mini. For plus use get_operation in a loop.',
    },
    {
        name: 'list_worlds', group: 'world',
        description: 'Paginated list of all generated worlds',
        args: { page_size: 'int (default 20, max 100)', page_token: 'str (optional)' },
...
export function Help() {
    ...
    const filtered = group === 'all' ? TOOLS : TOOLS.filter(t => t.group === group);
        ...
        <div className="space-y-6 page-enter max-w-3xl mx-auto">
            ...
            <div className="flex items-center gap-3">
                ...
                <div>
                    ...
                    <p className="text-sm text-slate-500 mt-0.5">worldlabs-mcp · Marble API reference</p>
                </div>
            </div>
            ...
            <Section title="What is this?" icon={Info} defaultOpen={true}>
                <p className="text-sm text-slate-400 leading-relaxed">
                    <strong className="text-slate-200">worldlabs-mcp</strong> is an MCP server that exposes the{' '}
                    <a href="https://docs.worldlabs.ai/api" target="_blank" rel="noopener noreferrer" className="text-cosmos-400 hover:text-cosmos-300 transition-colors">World Labs Marble API</a>{' '}
                    as tools callable by Claude (or any MCP client). It generates navigable 3D scenes — Gaussian splats, collision meshes, panoramas — from text descriptions, images, and video.
                </p>
                <p className="text-sm text-slate-400 leading-relaxed">
                    This webapp (port 10864) is the management dashboard. The MCP server itself runs as a separate process (stdio for Claude Desktop, HTTP for testing). The FastAPI bridge on port 10865 proxies requests from this UI to the Marble API.
                </p>
                ...
                <a href="https://platform.worldlabs.ai/api-keys" target="_blank" rel="noopener noreferrer"
            ...
            <Section title="World Labs — The Company &amp; Marble API" icon={Globe2} defaultOpen={false}>
                <div className="space-y-3 text-sm text-slate-400 leading-relaxed">
                    <p>
                        World Labs was founded in 2023 in San Francisco by <strong className="text-slate-300">Fei-Fei Li</strong> (former director of the Stanford AI Lab and Google Cloud AI), Justin Johnson, and Christoph Lassner. Its stated mission is to build <em>spatial intelligence</em> — AI that understands the three-dimensional structure of the world, not just text and flat images.
                    </p>
                    <p>
                        The company emerged from a body of academic work on 3D vision, neural rendering, and scene reconstruction. Fei-Fei Li had long argued that current language models are fundamentally limited by their inability to reason about space, objects, and causality the way a physical agent does. World Labs is the commercial vehicle for addressing that gap.
                    </p>
                    <p>
                        <strong className="text-slate-300">Marble</strong> is the production API. It accepts a text prompt, a single image, multiple images at azimuth angles, or a video clip, and returns a fully navigable 3D Gaussian splat scene. Internally it runs a pipeline of: scene understanding → geometry estimation → Gaussian splatting → compression → asset packaging.
                    </p>
                    ...
                    <p className="text-xs text-slate-500 border-l border-white/10 pl-3 italic">
                        The Marble gallery at worldlabs.ai/gallery shows publicly shared worlds. Downloading from the gallery is an interactive browser task — there is no API endpoint for it. Use the world detail page to download SPZ directly once you have a world_id.
                    </p>
                </div>
            </Section>

            {/* ---- Spatial Intelligence Scene 2026 ---- */}
            <Section title="Spatial Intelligence Landscape (2026)" icon={Layers} defaultOpen={false}>
                <div className="space-y-3 text-sm text-slate-400 leading-relaxed">
                    <p>
                        World Labs operates in the Large World Model (LWM) space, which in 2026 has several distinct technical approaches competing for different use cases.
                    </p>
                    ...
                    <p className="text-xs text-slate-500 leading-relaxed">
                        World Labs positions itself at the quality/persistence end: scenes you can download, import into Blender, and use in production, rather than scenes that only exist inside a model's latent space during inference.
                    </p>
                </div>
            </Section>
            ...
            {/* ---- Gallery Download ---- */}
            <Section title="Gallery &amp; Download" icon={Download} defaultOpen={false}>
                <div className="space-y-2 text-sm text-slate-400 leading-relaxed">
                    <p>
                        The <a href="https://worldlabs.ai/gallery" target="_blank" rel="noopener noreferrer" className="text-cosmos-400 hover:text-cosmos-300 transition-colors inline-flex items-center gap-1">Marble gallery <ExternalLink className="w-3 h-3" /></a> shows publicly shared worlds. There is no API endpoint for browsing or downloading from the gallery — it requires browser interaction.
                    </p>
                    <p className="text-xs text-slate-500">
                        To download a world you generated: use <code className="bg-white/[0.06] px-1 rounded text-[11px]">get_world(world_id)</code> to retrieve the SPZ and GLB URLs, then download directly. CDN links are time-limited — download within a few hours of generation.
                    </p>
                    <a href="https://worldlabs.ai/gallery" target="_blank" rel="noopener noreferrer"
                        className="inline-flex items-center gap-1.5 text-xs text-cosmos-400 hover:text-cosmos-300 transition-colors">
                    ...
                    <div className="flex gap-1.5">
                        {(['quick', 'standard', 'verbose'] as DetailLevel[]).map(d => (
                            <button
                                key={d}
                                onClick={() => setDetail(d)}
                                className={`px-2.5 py-1 rounded text-xs font-medium transition-colors ${detail === d
                    ...
                    <div className="flex gap-1.5">
                        {(['all', ...GROUPS] as string[]).map(g => (
                            <button
                                key={g}
                                onClick={() => setGroup(g)}
                                className={`px-2.5 py-1 rounded text-xs transition-colors ${group === g
