        ...
        <div className="mt-2 space-y-2 pl-5 border-l border-white/[0.06]">
          ...
          {detail === "verbose" && tool.example && (
            <div>
              ...
              <pre className="text-[11px] font-mono text-slate-300 bg-black/30 rounded p-2 overflow-x-auto">
                {tool.example}
              </pre>
            </div>
          )}
...
const TOOLS: Tool[] = [
  {
    ...
    docstring:
      "Submits a text-to-world request to the Marble API. Returns immediately with an in-progress operation. Poll with get_operation or block with wait_for_world.",
    example:
      'generate_world_from_text(\n  text_prompt="A gothic cathedral interior at night"\n)',
    notes:
      "Credits are consumed per generation. Check billing at platform.worldlabs.ai.",
  },
  {
    ...
    notes:
      "Image must be publicly accessible. GCS signed URLs are not supported here.",
  },
  {
    ...
    docstring:
      "Reconstructs a 3D scene from multiple views. 2-8 images at known azimuth angles for best results. Cardinal directions (0/90/180/270) work well.",
    example:
      'generate_world_from_multi_image(\n  image_urls=["north.jpg", "south.jpg"],\n  azimuths_deg=[0, 180]\n)',
    notes:
      "Minimum 2 images. image_urls and azimuths_deg must have equal length.",
  },
  {
  ...
  },
  {
    ...
    docstring:
      "Handles the full GCS signed-upload flow then generation in one call. Supports jpg, jpeg, png, webp (image); mp4, mov, mkv (video).",
    example:
  ...
  },
  {
    ...
    docstring:
      "Step 1 of the manual upload flow. PUT the raw bytes to upload_info.upload_url, then call generate_world_from_media_asset. Prefer upload_and_generate for simpler usage.",
    example:
  ...
  },
  {
    ...
    docstring:
      "One-shot poll. Returns immediately with current state. Call in a loop for marble-1.1-plus jobs (often multi-minute). Preferred over wait_for_world for plus jobs.",
    example: 'get_operation("op-abc123")',
    notes:
      "Check the 'done' field. If done, 'response' contains the world data.",
  },
  {
    name: "wait_for_world",
    group: "poll",
    ...
    docstring:
      "Blocks until done or timeout. Raises RuntimeError on API failure, TimeoutError if timeout elapses. Default 90s is safe for marble-1.1. For plus use get_operation in a loop.",
    example:
  ...
  },
  {
    ...
    docstring:
      "Returns splat URLs (SPZ), collision mesh (GLB), panorama, thumbnail, AI caption, and Marble viewer link.",
  ...
  },
  {
    ...
    docstring:
      "Permanently removes the world and all associated assets. This action cannot be undone.",
  ...
  },
  {
    ...
    group: "ui",
    description:
      "Paginated world library as a scannable table card (Prefab UI)",
  ...
  },
  {
    ...
    group: "ui",
    description:
      "Single world detail card with asset links and thumbnail (Prefab UI)",
    args: { world_id: "str" },
  ...
  },
  {
    ...
    args: {
      prompt_or_url: "str — URL to audio file",
      x: "float",
      y: "float",
      z: "float",
    },
  ...
  },
  {
    ...
    args: {
      avatar_url: "str — glTF URL or 'default_agent'",
      x: "float",
      y: "float",
      z: "float",
    },
...
export function Help() {
  ...
  return (
    <div className="space-y-6 page-enter max-w-3xl mx-auto">
      ...
      {/* ---- LLM Chat & Skills ---- */}
      <Section
        title="LLM Chat &amp; Skill Injection"
        icon={Brain}
        defaultOpen={true}
      >
        <div className="space-y-3 text-sm text-slate-400 leading-relaxed">
          <p>
            The <strong>Local LLM</strong> page at{" "}
            <code className="text-cosmos-400">/local-llm</code> provides an
            interactive AI chat running entirely on your local GPU via Ollama or
            LM Studio.
          </p>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-2">
            {[
              {
                label: "Expert",
                body: "Technically precise. References specific tool names and parameters. Default.",
              },
              {
                label: "Creative",
                body: "Artistic and evocative. Helps craft vivid world prompts with sensory language.",
              },
              {
                label: "Guide",
                body: "Patient tutor. Walks through workflows step by step with concrete examples.",
              },
              {
                label: "Concise",
                body: "Short, direct answers in 1-3 sentences. Prefers bullet points.",
              },
            ].map((p) => (
              <div key={p.label} className="glass-card p-3">
                <p className="text-[10px] font-bold uppercase tracking-wider text-cosmos-300">
                  {p.label}
                </p>
          ...
          <h4 className="text-xs font-bold uppercase tracking-wider text-slate-300 mt-2">
            Skill Injection
          </h4>
          <p>
            When the <strong>Marble Skill</strong> toggle is on, the World Labs
            Marble Expert skill is injected into the system prompt, giving the
            LLM detailed knowledge of all tools, models, output formats, prompt
            engineering, and pricing.
          </p>
          <p className="text-xs text-slate-500 border-l border-white/10 pl-3 italic">
            Conversation history is persisted per session and included as
            context for up to 10 previous exchanges.
          </p>
        </div>
      </Section>
