    ...
    <div className="flex flex-col h-[calc(100vh-120px)] space-y-4">
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
        <div>
          ...
          <p className="text-sm text-slate-400">Real-time system telemetry and bridge logs</p>
        </div>
            ...
            {(['ALL', 'INFO', 'WARN', 'ERROR', 'DEBUG'] as const).map(l => (
              <button
                ...
                className={cn(
                  ...
                  filter === l ? "bg-cosmos-500 text-white shadow-lg" : "text-slate-400 hover:text-slate-200"
                )}
              >
                {l}
              </button>
        ...
        <div className="px-4 py-2 bg-white/5 border-b border-white/5 flex items-center justify-between">
          ...
          <div className="flex items-center gap-3">
            ...
            >
              ...
              <span className="text-[10px] font-bold uppercase">{isPaused ? "Output Paused" : "Streaming"}</span>
            </button>
        ...
        >
          {filteredLogs.length === 0 ? (
            <div className="h-full flex flex-col items-center justify-center text-slate-600 space-y-2 opacity-50">
              ...
              <p className="uppercase tracking-[0.2em] text-[9px]">Awaiting system signals...</p>
            </div>
          ) : (
            filteredLogs.map((log) => (
              <div key={log.id} className="group flex gap-3 py-0.5 border-l border-transparent hover:border-cosmos-500/30 hover:bg-white/[0.02] transition-colors -ml-1 pl-1">
                <span className="text-slate-600 shrink-0 select-none">
                  {new Date(log.timestamp).toLocaleTimeString([], { hour12: false, fractionalSecondDigits: 3 })}
                </span>
                <span className={cn("font-bold shrink-0 min-w-[50px]", getLevelColor(log.level))}>
                  {log.level}
                </span>
                <span className={cn(
                  "shrink-0 px-1.5 py-0 rounded text-[9px] font-bold",
                  log.source === 'backend' ? "bg-indigo-500/10 text-indigo-400 border border-indigo-500/20" : "bg-teal-500/10 text-teal-400 border border-teal-500/20"
                )}>
