...
const TOOLS = [
	{ name: "generate_world_from_text", desc: "Generate from a text prompt" },
	{
		...
		desc: "Generate from image URL (jpg, png, webp)",
	},
	{
		name: "generate_world_from_video",
		desc: "Generate from video URL (mp4, mov, mkv)",
	},
	{ name: "get_operation", desc: "Poll an operation for status / result" },
	{
		name: "wait_for_world",
		desc: "Block until operation completes (with timeout)",
	},
	{ name: "get_world", desc: "Fetch details for a generated world" },
	{
		name: "prepare_media_upload",
		desc: "Get signed URL for local file upload",
	},
	{
		name: "generate_world_from_media_asset",
		desc: "Generate from pre-uploaded media asset",
	},
];
...
const MODELS = [
	{
		...
		time: "~5 minutes",
		quality: "High quality, detailed geometry",
	},
	{
		name: "Marble 0.1-mini",
		time: "~30–45 seconds",
		quality: "Fast preview, lower fidelity",
	},
];
	...
	const sections = [
		{
			...
			content: (
				<div className="space-y-3 text-sm text-slate-400 leading-relaxed">
					<p>
						World Labs MCP wraps the Marble API to generate explorable 3D
						spatial worlds from text descriptions, images, and video — directly
					...
					<ol className="list-decimal list-inside space-y-1.5 text-slate-400">
						<li>
							...
							>
								platform.worldlabs.ai/api-keys
							</a>
						</li>
						<li>
							Set{" "}
							<code className="bg-white/[0.06] px-1.5 py-0.5 rounded font-mono text-xs">
								WORLDLABS_API_KEY
							</code>{" "}
							in your environment
						</li>
				...
				<div className="space-y-3">
					{TOOLS.map((t) => (
						<div key={t.name} className="flex items-start gap-2">
							...
							/>
							<div>
								<code className="text-xs font-mono text-cosmos-300">
									{t.name}
								</code>
			...
			<div className="flex items-center gap-3">
				...
				<div>
					...
					<p className="text-sm text-slate-500 mt-0.5">
						World Labs MCP — Marble API reference
					</p>
				</div>
			</div>
