...
export function WorldViewer() {
  ...
  function handleFileOpen(e: React.ChangeEvent<HTMLInputElement>) {
    ...
    const objectUrl =
      URL.createObjectURL(file) + (file.name.endsWith(".spz") ? "#.spz" : "");
  ...
  return (
    ...
    >
      ...
      <div className="glass-card p-3 flex flex-wrap items-center gap-2 flex-shrink-0">
        ...
        {/* Status badge */}
        {status === "loading" && (
          <span className="badge-pending animate-pulse">Loading…</span>
        )}
      ...
      >
        ...
        {status === "error" && (
          <div className="absolute inset-0 flex items-center justify-center bg-black/60 z-10">
            <div
              className={cn("glass-card p-6 max-w-md text-center space-y-2")}
            >
              <AlertCircle
              ...
              <p className="text-xs text-slate-600">
                SPZ files are compressed PLY. If the file fails, convert to .ply
                first using{" "}
                <a
                  href="https://spz.world"
                  target="_blank"
