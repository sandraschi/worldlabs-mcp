...
function GenerationModal({ operationId, onComplete, onDismiss }: GenerationModalProps) {
    ...
    const description = lastEvent?.description ?? 'Initialising generation pipeline…';
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/70 backdrop-blur-sm">
            ...
            >
                ...
                <h3 className="text-center text-lg font-bold text-white mb-1">
                    {done ? 'World Ready!' : failed ? 'Generation Failed' : 'Generating World…'}
                </h3>
                ...
                <p className="text-center text-sm text-slate-400 mb-5 min-h-[2.5rem] leading-relaxed px-2">
                    {done
                        ...
                        : failed
                            ? (lastEvent?.description ?? 'An error occurred during generation.')
                            : description}
                </p>
                ...
                <div className="flex items-center justify-between text-[11px] text-slate-500 mb-4">
                    ...
                    <span className="font-mono truncate max-w-[160px]" title={operationId}>
            ...
            <div className="flex items-start justify-between gap-2">
                ...
                <div className="flex items-center gap-1 opacity-40 group-hover:opacity-100 transition-opacity">
                    <button
                        ...
                        className={cn("p-1 rounded hover:bg-white/10 transition-colors", prompt.fave ? "text-pink-500" : "text-slate-400")}
                        aria-label="Favorite"
                        title="Favorite"
                    >
                        <Heart className={cn("w-3.5 h-3.5", prompt.fave && "fill-current")} />
                    </button>
                    <button
                        onClick={() => onUpdate(prompt.id, { star: !prompt.star })}
                        className={cn("p-1 rounded hover:bg-white/10 transition-colors", prompt.star ? "text-amber-400" : "text-slate-400")}
                        aria-label="Star"
                        title="Star"
                    >
                        <Star className={cn("w-3.5 h-3.5", prompt.star && "fill-current")} />
                    </button>
                    <button
...
function AssetPanel({ op, assets }: { op: Operation; assets: FlatAssets }) {
    ...
    async function handleHandoff(target: 'resonite' | 'unity3d' | 'blender', type: 'splat' | 'mesh') {
        const key = `${target}-${type}`;
        setEs(key, 'loading');
        try {
            const assetUrl = type === 'splat' ? (assets.splat_500k || assets.splat_full) : assets.mesh;
    ...
    const handoffs: Array<{ target: 'resonite' | 'unity3d' | 'blender'; label: string; icon: string }> = [
            ...
            <div>
                ...
                <div className="flex flex-wrap gap-2">
                    {handoffs.map(({ target, label, icon }) => {
                        ...
                        return (
                            <div key={target} className="flex gap-1 p-0.5 bg-white/[0.03] rounded-lg border border-white/[0.06]">
                                ...
                                <div className="flex gap-1 p-0.5">
                                    <button
                                        ...
                                        disabled={splatState === 'loading' || !(assets.splat_500k || assets.splat_full)}
                                        className={cn(
                                            'px-2 py-1 rounded text-xs font-medium transition-all',
                                            splatState === 'loading' && 'opacity-60 cursor-not-allowed',
                                            splatState === 'ok' && 'bg-aurora-500/20 text-aurora-300',
                                            splatState === 'error' && 'bg-red-500/20 text-red-300',
                                            splatState === 'idle' && 'hover:bg-white/[0.08] text-slate-350 hover:text-white',
                                    ...
                                    <button
                                        ...
                                        className={cn(
                                            'px-2 py-1 rounded text-xs font-medium transition-all',
                                            meshState === 'loading' && 'opacity-60 cursor-not-allowed',
                                            meshState === 'ok' && 'bg-aurora-500/20 text-aurora-300',
                                            meshState === 'error' && 'bg-red-500/20 text-red-300',
                                            meshState === 'idle' && 'hover:bg-white/[0.08] text-slate-350 hover:text-white',
...
function OperationCard({
    ...
    const status = isDone ? (op.error ? 'FAILED' : 'SUCCEEDED') : progress?.status || 'IN_PROGRESS';

    return (
        <div
            className={cn(
                'glass-card p-4 transition-all duration-500',
                isDone && !op.error ? 'border-aurora-500/30 shadow-[0_0_15px_-5px_rgba(16,185,129,0.1)]' : '',
                ...
                <div className="flex gap-3 min-w-0">
                    <div className="mt-1">
                        {status === 'SUCCEEDED' ? (
                            ...
                            <RefreshCw className="w-4 h-4 text-cosmos-400 animate-spin" aria-hidden="true" />
                        )}
                    </div>
                    <div className="min-w-0">
                        <h4 className="text-sm font-medium text-white truncate">
                            {op.response?.display_name || `World ${op.operation_id.slice(0, 8)}`}
                        </h4>
                        <div className="flex items-center gap-2 mt-1">
                            ...
                            {progress?.description && (
                                <span className="text-xs text-slate-400 truncate">{progress.description}</span>
                            )}
                        </div>
                    </div>
                </div>
...
export default function WorldGenPage() {
    ...
    useEffect(() => {
        if (historyData) {
            ...
            const unfinished = historyData.filter(op => !op.done).map(op => op.operation_id);
    ...
    const updateOp = useCallback((updated: Operation) => {
        setOperations(prev => prev.map(o => (o.operation_id === updated.operation_id ? updated : o)));
    }, []);
    ...
    const [selectedLlm, setSelectedLlm] = useState<{ provider: string; id: string } | null>(null);
    ...
    useEffect(() => {
        if (availableModels.length > 0 && !selectedLlm) {
            setSelectedLlm({ provider: availableModels[0].provider, id: availableModels[0].id });
        }
    ...
    const handleModalComplete = useCallback((completedOp: Operation) => {
    ...
    const savePromptMutation = useMutation({
        mutationFn: (text: string) => api.createPrompt({ text, style: selectedStyle }),
        onSuccess: () => refetchPrompts(),
    });

    const updatePromptMutation = useMutation({
        mutationFn: ({ id, update }: { id: string; update: any }) => api.updatePrompt(id, update),
        onSuccess: () => refetchPrompts(),
    });
                ...
                <div className="max-w-4xl mx-auto space-y-8 pb-20">
                    ...
                    <div className="flex items-center gap-4 mb-2">
                        ...
                        <div>
                            ...
                            <p className="text-sm text-slate-400 mt-0.5">Generate 3D worlds with the Marble API</p>
                        </div>
                    </div>
                    ...
                    <div className="glass-card p-5 space-y-4">
                        ...
                        <div className="space-y-2">
                            <div className="flex items-center justify-between">
                                ...
                                {availableModels.length > 0 && (
                                    <div className="flex items-center gap-2">
                                        ...
                                        >
                                            {['Cinematic', 'Cyberpunk', 'Photorealistic', 'Surreal', 'Vibrant', 'Dark', 'Ethereal'].map(s => (
                                                <option key={s} value={s}>{s}</option>
                                            ))}
                                        </select>
                                        <select
                                            aria-label="Select local LLM for prompt refinement"
                                            value={selectedLlm ? `${selectedLlm.provider}:${selectedLlm.id}` : ''}
                                            onChange={(e) => {
                                        ...
                                        <button
                                            ...
                                            className={cn(
                                                ...
                                                (refineMutation.isPending || !prompt.trim()) && "opacity-50 cursor-not-allowed"
                                            )}
                                        >
                            ...
                            {refineMutation.error instanceof Error && (
                                <p className="text-[10px] text-red-400 mt-1">{refineMutation.error.message}</p>
                            )}
                        </div>
                                    ...
                                    <label className="flex items-center gap-2 cursor-pointer group mt-1">
                                        <div
                                            ...
                                            className={cn(
                                                ...
                                                isPanorama ? 'bg-cosmos-500 border-cosmos-500' : 'bg-black/40 group-hover:border-white/40',
                                            )}
                                        >
                                            {isPanorama && <div className="w-1.5 h-1.5 bg-white rounded-full" />}
                                        </div>
                        ...
                        {genMutation.error instanceof Error && (
                            <p className="text-xs text-red-400 mt-2 text-center">{genMutation.error.message}</p>
                        )}
                    </div>
                    ...
                    <div className="space-y-4">
                        <div className="flex items-center justify-between border-b border-white/[0.06] pb-2">
                            <div className="flex gap-4">
                                <button
                                    ...
                                    className={cn(
                                        ...
                                        !showLibrary ? "text-white border-b-2 border-cosmos-500" : "text-slate-500 hover:text-slate-300"
                                    )}
                                >
                                    Operations
                            ...
                            {isHistoryLoading && !showLibrary && <RefreshCw className="w-3 h-3 animate-spin text-slate-500" />}
                        </div>
                            ...
                            <div className="space-y-4">
                                {operations.length === 0 ? (
                                    <div className="glass-card py-12 flex flex-col items-center justify-center text-center px-6">
                                        ...
                                        <p className="text-sm text-slate-500">No active or past operations.</p>
                            ...
                            <div className="space-y-4">
                                {!savedPrompts || savedPrompts.length === 0 ? (
                                    <div className="glass-card py-12 flex flex-col items-center justify-center text-center px-6">
                                        ...
                                        <p className="text-xs text-slate-600 mt-1 max-w-[200px]">
                                            Click the <Save className="w-2.5 h-2.5 inline" /> icon after refining a prompt to save it here.
                                        </p>
                                    </div>
                                ) : (
                                    <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                                        {savedPrompts.map(p => (
                                            <PromptCard
                                                ...
                                                onUpdate={(id, update) => updatePromptMutation.mutate({ id, update })}
