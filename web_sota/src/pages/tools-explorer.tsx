...
import { Wrench, Search, ChevronDown, ChevronRight, Code2 } from 'lucide-react';
import { useState } from 'react';
import { api, type ToolInfo } from '@/lib/api';
import { cn } from '@/lib/utils';

function ToolCard({ tool, expanded, onToggle }: {
    tool: ToolInfo; expanded: boolean; onToggle: () => void;
}) {
    return (
        <div className={cn(
            ...
            >
                ...
                <div className="flex-1 min-w-0">
                    <div className="text-sm font-semibold text-slate-200 font-mono">{tool.name}</div>
                    <div className="text-xs text-slate-500 truncate mt-0.5 line-clamp-1">{tool.description}</div>
                </div>
                {expanded
            ...
            {expanded && (
                <div id={`tool-${tool.name}`} className="px-4 pb-4 space-y-3 border-t border-white/[0.04] pt-4">
