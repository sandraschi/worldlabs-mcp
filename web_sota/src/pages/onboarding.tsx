			...
			<div className="flex items-center gap-3">
				...
				<div>
					...
					<p className="text-sm text-slate-400">
						Connect your Quest, Pico, or Vive for wireless 3D streaming.
					</p>
				</div>
			</div>
								...
								{adbData &&
									...
									(adbData.devices && adbData.devices.length > 0 ? (
										...
										<div className="text-xs text-slate-500">
											No devices connected. Connect via USB and check the
											headset for the allow prompt.
										</div>
									))}
							</div>
						)}
							...
							<div className="glass-card p-4 border-amber-500/20 space-y-2">
								{step.id === "wireless" && (
									<div className="flex items-center justify-between p-3 rounded-lg bg-black/40 border border-white/[0.06]">
										<code className="text-xs font-mono text-cosmos-200">
											adb tcpip 5555
										</code>
										<button
											onClick={() => copyToClipboard("adb tcpip 5555", "tcpip")}
											className="p-1.5 rounded text-slate-500 hover:text-cosmos-400 transition-all"
										>
											<ShieldCheck
												className={cn(
													"w-4 h-4",
													copied === "tcpip" && "text-aurora-500",
												)}
											/>
										</button>
									</div>
									...
									<div className="flex items-center justify-between p-3 rounded-lg bg-black/40 border border-white/[0.06]">
										...
										>
											<ShieldCheck
												className={cn(
													"w-4 h-4",
													copied === "connect" && "text-aurora-500",
												)}
											/>
										</button>
									</div>
									...
									<div className="flex items-center justify-between p-3 rounded-lg bg-black/40 border border-white/[0.06]">
										...
										>
											<ShieldCheck
												className={cn(
													"w-4 h-4",
													copied === "reverse" && "text-aurora-500",
												)}
											/>
										</button>
									</div>
								...
								<div className="flex items-center gap-2 mt-1">
									...
									>
										<ShieldCheck
											className={cn(
												"w-4 h-4",
												copied === "flags" && "text-aurora-500",
											)}
										/>
									</button>
								</div>
								<p className="text-[10px] text-amber-500/60">
									Add http://localhost:10864 to the list, then relaunch the
									browser.
								</p>
							</div>
						)}
			...
			<details className="glass-card">
				...
				<div className="px-4 pb-4 space-y-1">
					{[
						...
						{
							cmd: "adb connect 192.168.1.XXX:5555",
						...
						<div
							key={item.cmd}
							className="flex items-center gap-2 p-2 rounded bg-white/[0.02]"
						>
							<code className="text-[10px] font-mono text-cosmos-300 flex-1">
								{item.cmd}
							</code>
