...
import { API_BASE, api, type LlmModel } from "@/lib/api";
import { cn } from "@/lib/utils";
import {
  PERSONALITIES,
  type PersonalityId,
  WORLDLABS_EXPERT_SKILL,
} from "@/skills/worldlabs-expert";
    ...
    <div className="glass-card p-5 space-y-4">
        ...
        <div className="flex items-center gap-3">
          ...
          >
            <Cpu
              className={cn(
                "w-4 h-4",
                available ? "text-aurora-400" : "text-slate-500",
              )}
              aria-hidden="true"
            />
          </div>
          <div>
            ...
            {url && (
              <div className="text-xs text-slate-500 font-mono">{url}</div>
            )}
          </div>
        </div>
        {available ? (
      ...
      {!available && (
        <div className="text-sm text-slate-500 space-y-2">
          ...
          <a
            href={
              name === "Ollama" ? "https://ollama.ai" : "https://lmstudio.ai"
            }
            target="_blank"
            rel="noopener noreferrer"
            className="inline-flex items-center gap-1.5 text-xs text-cosmos-400 hover:text-cosmos-300"
          >
...
export function LocalLlm() {
  ...
  const [chatProvider, setChatProvider] = useState(
    () => localStorage.getItem("llm_chat_provider") || "ollama",
  );
  const [personality, setPersonality] = useState<PersonalityId>(
    () =>
      (localStorage.getItem("llm_personality") as PersonalityId) || "expert",
  );
  const [injectSkill, setInjectSkill] = useState(
    () => localStorage.getItem("llm_inject_skill") !== "false",
  );
    ...
    <div className="space-y-6 page-enter max-w-3xl mx-auto">
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-sm text-slate-500 mt-0.5">
            Chat with models on your RTX 4090
          </p>
        </div>
        <button
          onClick={() => void refetch()}
          className="flex items-center gap-2 px-3 py-1.5 rounded-lg text-sm text-slate-400 hover:text-white bg-white/[0.04] hover:bg-white/[0.08] border border-white/[0.06]"
        >
      ...
      <div className="glass-card p-5 border-void-500/20 bg-gradient-to-br from-void-900/20 to-transparent">
        <div className="flex items-center gap-3 mb-4">
          <Zap className="w-4 h-4 text-void-400" />
          <h3 className="text-sm font-bold text-slate-200">
            RTX 4090 Model Recommendations
          </h3>
        </div>
          ...
          {GPU_RECS.map((r) => (
            ...
            >
              ...
              <div className="flex items-center gap-2">
                ...
                <CopyButton
                  text={`ollama pull ${r.model.split(" ")[0].toLowerCase()}`}
                />
              </div>
            </div>
          ))}
        ...
        <p className="text-xs text-slate-600 mt-2">
          4-bit quantization on 24 GB GDDR6X.
        </p>
      </div>
      ...
      {allModels.length > 0 && (
        <div className="glass-card p-5 space-y-4">
          <div className="flex items-center justify-between flex-wrap gap-2">
            ...
            <div className="flex items-center gap-2">
              <select
                className="bg-slate-800 border border-slate-600 rounded text-xs px-2 py-1.5 text-slate-300 max-w-[180px]"
                value={chatModel}
          ...
          <div className="flex flex-wrap items-center gap-3">
            <div className="flex items-center gap-1.5">
              ...
              <div className="flex gap-1">
                {(
                  Object.entries(PERSONALITIES) as [
                    PersonalityId,
                    (typeof PERSONALITIES)[PersonalityId],
                  ][]
                ).map(([id, p]) => (
                  <button
                    key={id}
          ...
          <div
            className="h-72 overflow-y-auto space-y-2 text-sm pr-1"
            style={{ scrollBehavior: "smooth" }}
          >
            {chatMessages.length === 0 && (
              <p className="text-slate-500 text-xs text-center pt-16">
            ...
            {chatMessages.map((msg, i) => (
              ...
              >
                <div
                  className={`max-w-[85%] rounded-xl px-3 py-2 whitespace-pre-wrap ${msg.role === "user" ? "bg-cosmos-800 text-cosmos-100" : "bg-slate-800 text-slate-300"}`}
                >
                  {msg.content}
                </div>
              </div>
          ...
          <div className="flex gap-2">
            <input
              className="flex-1 bg-slate-800 border border-slate-600 rounded-lg px-3 py-2 text-sm text-slate-200 focus:outline-none focus:border-cosmos-500"
            ...
            <button
              ...
              className="bg-cosmos-700 hover:bg-cosmos-600 disabled:bg-slate-700 text-white px-3 py-2 rounded-lg text-sm font-medium"
            >
      ...
      {isError && (
        <div className="glass-card p-4 border-amber-500/20 text-sm text-amber-400">
          Backend not reachable — start the bridge server.
        </div>
      )}
