...
import {
    ...
    ChevronDown, RefreshCw, Loader2, CheckCircle2,
    XCircle, Usb, WifiIcon,
} from 'lucide-react';
...
export function ImmersiveDetail() {
    ...
    return (
        <div className="space-y-4 page-enter max-w-4xl mx-auto pb-20">
            ...
            <div className="glass-card p-6 relative overflow-hidden">
                ...
                <div className="relative flex items-center gap-4">
                    ...
                    <div>
                        ...
                        <p className="text-sm text-slate-400">Cross-reality streaming, device debugging, and spatial orchestration.</p>
                    </div>
                </div>
            </div>
            ...
            <Section title="Protocols & Compatibility" icon={Zap} defaultOpen={false}>
                <div className="grid gap-3 md:grid-cols-2">
                    <div className="glass-card p-4 space-y-2">
                        ...
                        <p className="text-xs text-slate-500 leading-relaxed">
                            Optimises LoD splat tree streaming for 6DOF headsets with bitrate-aware progressive refinement.
                        </p>
                    </div>
                    <div className="glass-card p-4 space-y-2">
                        <div className="flex items-center gap-2">
                            <Smartphone className="w-4 h-4 text-cosmos-400" />
                            <h3 className="text-xs font-bold text-white">Android XR Native</h3>
                        </div>
                        <p className="text-xs text-slate-500 leading-relaxed">
                            Native hand-tracking, shared spatial anchors, multithreaded splat sorting for Quest 3 & Pico 4.
                        </p>
                    </div>
                    <div className="glass-card p-4 space-y-2">
                        <div className="flex items-center gap-2">
                            <Wifi className="w-4 h-4 text-amber-400" />
                            <h3 className="text-xs font-bold text-white">Sovereign Handoff</h3>
                        </div>
                        <p className="text-xs text-slate-500 leading-relaxed">
                            Stream directly from your workstation (port 10865) to HMD with zero-latency handshakes.
                        </p>
                    </div>
                    <div className="glass-card p-4 space-y-2">
                        <div className="flex items-center gap-2">
                        ...
                        <p className="text-xs text-slate-500 leading-relaxed">
                            Stereo Passthrough v2, PicoOS hand-tracking, WiFi-7 optimised streaming.
                        </p>
                    </div>
                </div>
                <div className="grid gap-2 md:grid-cols-4">
                    {[
                        ...
                        { name: 'Apple Vision Pro', os: 'visionOS 3.0', status: 'WebGL2', color: 'text-cosmos-400' },
                        ...
                        <div key={hmd.name} className="p-3 rounded-lg bg-white/[0.03] border border-white/[0.06] space-y-1">
            ...
            <Section title="Headset Connection Wizard" icon={Terminal} defaultOpen={true}>
                <div className="space-y-4">
                    <p className="text-xs text-slate-400 leading-relaxed">
                        Connect your Quest, Pico, or Vive headset to this workstation for wireless streaming.
                        Requires <strong>ADB</strong> (Android Platform Tools) installed on your system.
                    </p>
                        ...
                        {adbData && !adbData.success && (
                            <div className="flex items-start gap-2 text-xs text-red-400">
                                ...
                                <div>
                                    ...
                                    <p className="text-slate-500 mt-0.5">{adbData.error || 'Install Android Platform Tools and add adb to PATH.'}</p>
                        ...
                        {adbData && adbData.success && (
                            adbData.devices && adbData.devices.length > 0 ? (
                                ...
                                <div className="text-xs text-slate-500">
                                    No devices connected. Connect your headset via USB and ensure Developer Mode is enabled.
                                </div>
                            )
                        )}
                    </div>
                    ...
                    <div className="space-y-2">
                        {[
                            { id: 'adb-devices', cmd: 'adb devices', label: 'Verify connection', desc: 'List connected devices' },
                            { id: 'adb-tcp', cmd: 'adb tcpip 5555', label: 'Enable wireless mode', desc: 'Restarts ADB over TCP (requires USB first)' },
                            { id: 'adb-connect', cmd: `adb connect ${adbData?.devices?.[0]?.serial?.replace(/:.*$/, '') || '192.168.1.XX'}:5555`, label: 'Connect over WiFi', desc: 'Replace with your headset IP' },
                            { id: 'adb-reverse', cmd: 'adb reverse tcp:10864 tcp:10865', label: 'Port forward dashboard', desc: 'Access worldlabs-mcp from headset browser' },
                        ].map(item => (
                            <div key={item.id} className="flex items-center gap-2 p-3 rounded-lg bg-white/[0.03] border border-white/[0.06]">
                                <div className="flex-1 min-w-0">
                                    <div className="flex items-center gap-2">
                                        <span className="text-[10px] font-bold text-slate-500 uppercase">{item.label}</span>
                                        <span className="text-[10px] text-slate-600">— {item.desc}</span>
                                    </div>
                                    <code className="text-xs font-mono text-cosmos-200 mt-1 block truncate">{item.cmd}</code>
                                </div>
                                <button onClick={() => copyToClipboard(item.cmd, item.id)}
                    ...
                    <div className="p-4 rounded-xl bg-amber-500/5 border border-amber-500/20 text-xs text-slate-400 leading-relaxed space-y-2">
                        ...
                        <p>Open the headset browser and navigate to <code className="text-amber-200 bg-black/30 px-1 rounded">http://localhost:10864</code> to access the Spark 2.0 viewer. The "Enter VR" button requires a secure context — set <code className="text-amber-200 bg-black/30 px-1 rounded">chrome://flags/#unsafely-treat-insecure-origin-as-secure</code> on the headset browser.</p>
                    </div>
                </div>
            </Section>

            {/* Quick reference */}
            <Section title="Spark Viewer Controls" icon={HelpCircle} defaultOpen={false}>
                <div className="text-xs text-slate-400 space-y-2">
                    <div className="grid grid-cols-2 gap-2">
                        {[
                            ...
                            <div key={i.action} className="flex justify-between p-2 rounded bg-white/[0.03] border border-white/[0.06]">
                                <span className="text-slate-200">{i.action}</span>
                                <span className="text-slate-500 font-mono text-[10px]">{i.input}</span>
                            </div>
                        ))}
                    </div>
                </div>
