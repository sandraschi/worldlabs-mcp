...
import { api, streamOperation, type Operation, type OperationStreamEvent } from '@/lib/api';
...
export function PaintingPortals() {
    ...
    const generateMutation = useMutation({
        mutationFn: async (painting: Painting) => {
            ...
            setOpId(null);
            return api.generateImage(painting.imageUrl, painting.caption, painting.title, 'marble-1.1', false);
        },
        onSuccess: (data) => {
            ...
            streamOperation(
                ...
                (event) => {
                    ...
                    if (event.done && event.response) {
                        setCompletedWorld({ ...data, done: true, response: event.response });
                    }
                },
                () => {
                    ...
                    const poll = setInterval(async () => {
                        try {
                            ...
                            setStatus({
                                ...
                                status: (op.metadata?.progress?.status as OperationStreamEvent['status']) ?? 'IN_PROGRESS',
    ...
    const splatUrl = completedWorld?.response?._assets?.splat_full
        ...
        <div className="space-y-6 page-enter max-w-7xl mx-auto">
            ...
            <div className="flex items-center gap-3">
                ...
                <div>
                    ...
                    <p className="text-sm text-slate-400">
                        Step inside famous works of art. Each painting generates a 3D world you can walk through.
                    </p>
                </div>
            </div>
                ...
                <div className="glass-card p-6 border-amber-500/30">
                    <div className="flex items-start gap-4">
                        ...
                        <div className="flex-1 min-w-0">
                            <h3 className="text-sm font-bold text-white">{activePainting.title}</h3>
                            <p className="text-xs text-slate-400">{activePainting.artist} · {activePainting.year}</p>
                                ...
                                <div className="mt-3 space-y-2">
                                    <div className="flex items-center gap-2 text-xs">
                                        ...
                                        <span className="text-slate-300">
                                            {status?.description || 'Generating 3D world from painting...'}
                                        </span>
                                    </div>
                                    ...
                                    <div className="w-full h-1.5 rounded-full bg-white/[0.06] overflow-hidden">
                                        <div className="h-full rounded-full bg-gradient-to-r from-cosmos-500 to-amber-500 animate-progress-slide" style={{ width: '35%' }} />
                                    </div>
                                    <p className="text-[10px] text-slate-500">
                                        Marble is extrapolating the painted scene into navigable 3D space.
                                        Typical generation takes 1–5 minutes depending on scene complexity.
                                    </p>
                                </div>
                            )}
            ...
            <div className="glass-card p-4">
                <p className="text-xs text-slate-500 leading-relaxed">
                    <strong className="text-slate-300">How it works:</strong> Each painting is fed as an image to the Marble API,
                    which extrapolates the 2D scene into a navigable 3D Gaussian splat world.
                    Paintings with strong perspective, depth cues, and landscape elements produce the most immersive results.
                    Uses <strong>marble-1.1</strong> (1500 credits per generation — separate from your web subscription).
                </p>
            </div>
        </div>
    );
